...
{
    /*public static async Task<T> LoadGameObject<T>(AssetReferenceGameObject reference) where T : MonoBehaviour
    {
        var handle = Addressables.InstantiateAsync(reference.RuntimeKey);
    ...
    public static async Task<T> LoadGameObject<T>(AssetReferenceGameObject reference) where T : MonoBehaviour
    {
        T result = await LoadGameObject<T>(reference.RuntimeKey.ToString());

        return result;
    }

    public static async Task<T> LoadGameObject<T>(string reference) where T : MonoBehaviour
    {
